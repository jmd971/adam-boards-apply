...
**Régression corrigée 2026-07-21 — bouton « Ajouter un compte »** : la feature d'ajout manuel d'un compte hors FEC (`f3d8b5a`, 2026-05-19 : états `showAddAccount`/`newAccNum`/`newAccLabel`/`newAccType` + `handleAddAccount` + panneau vert dans la toolbar) avait été **perdue par `78799f9`** (2026-06-15, réécriture du fichier depuis une base antérieure) — tout comme l'affichage en ordre d'exercice de `cdfa093`. Restaurée. **Avant toute réécriture large de `Budget.tsx`, diff contre la version en ligne pour vérifier qu'aucune feature n'est perdue** (checklist : Ajouter un compte, ordre d'exercice, sous-comptes, Recopier, hypothèses, groupes PCG, versions, what-if, détail écritures). L'en-tête du tableau budget est **figé** : `maxHeight + overflowY:auto` sur le conteneur et `position:sticky; top:0` sur **chaque `th`** (pas sur le `tr` — non fiable sous Chrome) ; la cellule coin « Compte » cumule `left:0` + `top:0` avec un zIndex supérieur.
