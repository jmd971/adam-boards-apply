...
export function Saisie() {
  ...
  const handleSubmit = async () => {
    ...
    const { data, error } = await sb.from('manual_entries').insert(payload).select().single()
    setSaving(false)
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {entries.slice(0, 30).map(e => (
            <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 12px', borderRadius: 8, background: '#0f172a', border: '1px solid rgba(255,255,255,0.04)', fontSize: 11 }}>
              ...
              <span style={{ minWidth: 60, padding: '2px 7px', borderRadius: 20, flexShrink: 0,
              ...
              <span style={{ fontFamily: 'monospace', fontWeight: 600, flexShrink: 0,
                color: e.category === 'Vente' ? '#10b981' : '#f1f5f9' }}>
